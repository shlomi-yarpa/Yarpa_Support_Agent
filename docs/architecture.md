...
SQL Server, **DB ייעודי נפרד** (Diagnostics DB) — מבודד מה-SQL/CRM הקיים של החברה כדי
לא לגעת במערכת הקיימת. גישת **append-only** – כל שליחה יוצרת Snapshot חדש; אין דריסה.
...
## 4. שכבת ה-CRM Dashboard ואינטגרציה

ה-Dashboard (Blazor Server) עצמאי אך **ניתן-להטמעה** בתוך ה-CRM הקיים (שהוא web).
מסכים: Summary, Timeline, Alerts (ראה [specification.md](specification.md)).

**קישור לרשומת הלקוח הקיימת** נעשה בצד השרת: לכל `Customer` שדה `ExternalCustomerCode`
= קוד הלקוח ב-CRM. ה-API Key שייך ל-Customer, כך שה-Agent נשאר מינימלי (API Key בלבד,
ללא קונפיגורציית קוד-לקוח פר-מחשב) והמיפוי מתבצע פעם אחת בשרת.

**דרכי הטמעה ב-CRM ה-web:**
- לשונית/עמוד "אבחון" ב-CRM שמטמיע את ה-Dashboard ב-iframe עם deep-link לפי הקוד
  (למשל `/diagnostics/{externalCustomerCode}`) וטוקן אימות משותף. או
- ה-CRM צורך ישירות את ה-read API של המערכת ומרנדר בעצמו.
...
- **DB ייעודי + מיפוי חיצוני** – נתוני ה-Diagnostics ב-DB נפרד מבודד; הקישור ל-CRM נעשה
  דרך `Customer.ExternalCustomerCode` בצד השרת, כך שאין תלות במבנה ה-DB של ה-CRM והאינטגרציה
  רופפת (loose coupling) וניתנת להחלפה.
